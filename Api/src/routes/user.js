...
server.post('/create', (req, res)=>{
    ...
    if (first_name && last_name && password  && email){
       rol ? 
            p1 = User.create({first_name, last_name, password: bcrypt.hashSync(password,10), email, rol, reset, ban}) :
            p2 = User.create({first_name, last_name, password: bcrypt.hashSync(password,10), email, rol:'user',reset, ban})
         /*    .then((user)=>{
                token = jwt.sign({
                    user
...
server.post('/sendMailPass', (req,res)=>{
    ...
    htmlContent = `<h2>Restablecer contraseña olvidada</h2>
            <div>
              ...
              <a href='https://bomba-ecommerce.web.app/ResetPassowordEmail?pass=${ans.password}' >ingrese a este link para restablecer su contraseña:</a>
              <p>https://bomba-ecommerce.web.app/ResetPassowordEmail?pass=${ans.password}</p>
            </div>
          `; 
