...
import {verReview, getNumProducts,setProductInfo} from '../../../../store/actions/index';
...
function Product({products, verReview, getNumProducts, setProductInfo}) {
   ...
   // const [flagCrud, setFlagCrud] = useState(false);
    ...
    if(localStorage.getItem('idPord')) setProductInfo() 
    ...
    <div className={s.product} >
      <div style={{width:400, height:350}} id="carouselExampleControls" wrap data-interval="5000" className={`carousel slide ${s.slide}`} data-ride="carousel"> 
        <div className={`carousel-inner ${s.foto}`}>
          {images &&
            <div className="carousel-item active" >
              <img  style={{width:350}} className="d-block w-100"  src= { images[0] ? `https://bomba-ecommerce.herokuapp.com${images[0].imgUrl}` : null} alt='sin imagen'/>
            </div>}
            {
            
            imagesfilter ?  imagesfilter.map(img => 
              <div style={{width:350}} className="carousel-item" >
                <img style={{width:350}}  src= {`https://bomba-ecommerce.herokuapp.com${img.imgUrl}`} alt='sin imagen'/>
              </div>) : null
          }
...
const mapStateToProps = (state) => {
  ...
  export default connect (mapStateToProps, {verReview, getNumProducts, setProductInfo})(Product);