...
function ProductCard({flagToken, setToken, setProductInfo, id, name, price, stock , images, ratePromedio, verReview, getNumProducts }) {
    useEffect(()=>{ },[flagToken])
        ...
        <main className={s.main}>
            <div className={s.product}>
                    <div className={s.circulo}>                   
                        <img key={images[0].id} className={s.imagen} src={images[0] ? `https://bomba-ecommerce.herokuapp.com${images[0].imgUrl}` :null} alt='sin imagen'/>
                    ...
                    <p className= {s.precio}> ${price} </p>
                  {
                    ...
                    <Link to="/products/" onClick={handleClick}><button className={s.botonText}> ver producto</button></Link>              
                    </div>
